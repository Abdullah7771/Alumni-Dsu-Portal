...
const profileDetailsSchema = Yup.object().shape({
  ...
  uni_email: Yup.string().required(' Uni Email is required'),
...
const ProfileDetails: React.FC = () => {
  ...
  const uploadFile = async (certificate: File | null, type: string): Promise<any> => {
    try {
      if (certificate && type == 'profilePic') {
        const formData = new FormData()
        formData.append('file', certificate)
        const response = await axios.post(
          `https://amsbackend-ghub.onrender.com/users/${localid}/uploadProfilePic`,
          formData
        )

        const data2 = response.data

        // handleImageClick(`https://amsbackend-ghub.onrender.com/alumni/${img}`)

        console.log(data2)

        return data2
      } else if (certificate && type == 'resume') {
        const formData = new FormData()
        formData.append('file', certificate)

        const response = await axios.post(
          `https://amsbackend-ghub.onrender.com/profiles/${data.profile.id}/uploadResume`,
          formData
        )
  ...
  const editRecordByUserId = async () => {
    try {
      console.log(data)
      const {email, uni_email, phone, first_name, middle_name, last_name} = data
      const {country, date_of_birth, timezone} = data.profile
      const resData = {
        email,
        uni_email,
        phone,
      ...
      const response = await axios.patch(
        `https://amsbackend-ghub.onrender.com/users/${localid}/withProfile`,
        resData,
        {
          headers: {
  ...
  const handleFormSubmit = async (values: IProfileDetails2) => {
    try {
      if (!formik.dirty) {
  ...
  return (
    <div className='card mb-5 mb-xl-10'>
      ...
      <div id='kt_account_profile_details' className='collapse show'>
        <div className='symbol symbol-100px symbol-lg-160px symbol-fixed position-relative mb-3 ms-5 ps-3'>
          <img
            src={`https://amsbackend-ghub.onrender.com/alumni/${img}`}
            alt='Metronic'
            onClick={() => handleImageClick(`https://amsbackend-ghub.onrender.com/alumni/${img}`)}
        ...
        <div className='row mb-6 ms-5 ps-3'>
          <label className='col-lg-4 col-form-label fw-bold fs-6'>Resume</label>
          ...
          <div className='col-lg-8 fv-row'>
            {data.profile ? (
              ...
              <span className='fw-bold fs-6'>No File</span>
            )}
            <button
        ...
        <form onSubmit={formik.handleSubmit} noValidate className='form'>
          <div className='card-body border-top p-9'>
              ...
              <div className='modal-dialog'>
                <div className='modal-content'>
                  <div className='modal-header'>
                    <h5 className='modal-title' id='fileModalLabel'>
                      Upload a File
                    </h5>
                    <button
                      type='button'
                  ...
                  <div className='modal-body'>
                    ...
                    {/* {data.avatar ? <span>{data.avatar}</span> : <span>No File</span>} */}
                  </div>
                  <div className='modal-footer'>
                    ...
                    <button
                      ...
                      onClick={() => uploadFile(selectedFile, uploadType)}
                      data-bs-dismiss='modal'
                    >
                      Upload
                    </button>
            ...
            <div className='row mb-6'>
              <label className='col-lg-4 col-form-label required fw-bold fs-6'>Uni Email</label>
              ...
              <div className='col-lg-8 fv-row'>
                <input
                  ...
                  className='form-control form-control-lg form-control-solid'
                  placeholder='Email'
                  {...formik.getFieldProps('uni_email')}
                  disabled
                />
                {formik.touched.uni_email && formik.errors.uni_email && (
                  <div className='fv-plugins-message-container'>
                    <div className='fv-help-block'>{formik.errors.uni_email}</div>
                  </div>
                )}
              </div>
            </div>
        ...
        <div>
          ...
          <div className='modal d-block' tabIndex={-1} onClick={handleCloseModal}>
            <div className='modal-dialog modal-dialog-centered'>
              <div className='modal-content'>
                <div className='modal-body'>
                  <img src={selectedImage} className='img-fluid' alt='Modal Image' />
                </div>
              </div>
            </div>
          </div>
